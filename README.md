...
- [5. 端侧离线推理 —— 不联网也能跑](#5-端侧离线推理--不联网也能跑)
- [6. 技能与插件系统](#6-技能与插件系统)
- [7. 安全审计与审批](#7-安全审计与审批)
- [8. 定时任务与记忆系统](#8-定时任务与记忆系统)
- [9. 多通道集成](#9-多通道集成)
- [10. Linux 子系统](#10-linux-子系统)
- [技术架构](#技术架构)
- [快速开始](#快速开始)
- [项目结构](#项目结构)
- [文档](#文档)
...
| 📱 **端侧模型** | 基于 Google LiteRT-LM，8 个 2B 以下模型离线推理，随时可用 |
...
基于 Google **LiteRT-LM** 引擎，可在设备本地运行 8 个 2B 参数以下的模型，完全不依赖网络。
...
|------|--------|---------|
| Gemma3-1B | 1B | 通用对话 |
| Qwen2.5-1.5B | 1.5B | 中文对话 |
| DeepSeek-R1-Distill-Qwen-1.5B | 1.5B | 推理任务 |
| Phi-4-mini | ~2B | 代码/数学 |
| Qwen3-0.6B | 0.6B | 轻量任务 |
| TinyLlama-1.1B | 1.1B | 通用 |
| Gemma3-270M | 270M | 极轻量 |
| SmolLM-135M | 135M | 最低配置 |

### 特性

- **后端选择**：自动 / CPU / GPU / NPU
- **HuggingFace 镜像加速**：自动探测国内最快镜像，断点续传
- **虚拟 Provider**：下载后自动注册为模型供应商，切换即用
- **内存保护**：设备 RAM 不足时警告，后台 5 分钟自动卸载
- **同时只加载一个引擎**，切换时自动卸载旧引擎
