...
class SymbolicRetriever(
    ...
    suspend fun retrieve(query: String, agentId: String, limit: Int = 10): List<Pair<CommonMemoryEntry, Float>> {
        ...
        val poolSize = limit * 2
        val exactMatches = memoryDao.searchByExactPhrase(segmentedQuery, agentId, poolSize)
        val phraseMatches = memoryDao.searchByPhrasePrefix(segmentedQuery, agentId, poolSize)
